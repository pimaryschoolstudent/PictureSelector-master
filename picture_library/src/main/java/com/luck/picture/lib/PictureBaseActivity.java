...
public abstract class PictureBaseActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!config.camera) {
            setTheme(config.themeStyleId == 0 ? MResource.getIdentifier(getContext(),"style","picture_default_style") : config.themeStyleId);
        }
    ...
    private void initConfig() {
        ...
        if (PictureSelectionConfig.uiStyle != null) {
            ...
            if (!openWhiteStatusBar) {
                openWhiteStatusBar = AttrsUtils.getTypeValueBoolean(this, MResource.getIdentifier(getContext(),"attr","picture_statusFontColor"));
            }

            numComplete = config.isOpenStyleNumComplete;
            if (!numComplete) {
                numComplete = AttrsUtils.getTypeValueBoolean(this, MResource.getIdentifier(getContext(),"attr","picture_style_numComplete"));
            }

            config.checkNumMode = config.isOpenStyleCheckNumMode;
            if (!config.checkNumMode) {
                config.checkNumMode = AttrsUtils.getTypeValueBoolean(this, MResource.getIdentifier(getContext(),"attr","picture_style_checkNumMode"));
            }

            if (config.titleBarBackgroundColor != 0) {
                colorPrimary = config.titleBarBackgroundColor;
            } else {
                colorPrimary = AttrsUtils.getTypeValueColor(this, MResource.getIdentifier(getContext(),"attr","colorPrimary"));
            }

            if (config.pictureStatusBarColor != 0) {
                colorPrimaryDark = config.pictureStatusBarColor;
            } else {
                colorPrimaryDark = AttrsUtils.getTypeValueColor(this, MResource.getIdentifier(getContext(),"attr","colorPrimaryDark"));
            }
        }
    ...
    protected void exit() {
        finish();
        if (config.camera) {
            overridePendingTransition(0, MResource.getIdentifier(getContext(),"anim","picture_anim_fade_out"));
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == PictureConfig.APPLY_AUDIO_PERMISSIONS_CODE) {
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                ToastUtils.s(getContext(), getString(MResource.getIdentifier(getContext(),"string","picture_audio")));
            }
        }
    }
