...
public class PictureImageGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == PictureConfig.TYPE_CAMERA) {
            View view = LayoutInflater.from(context).inflate(MResource.getIdentifier(context,"layout","picture_item_camera"), parent, false);
            return new CameraViewHolder(view);
        } else {
            View view = LayoutInflater.from(context).inflate(MResource.getIdentifier(context,"layout","picture_image_grid_item"), parent, false);
    ...
    public void onBindViewHolder(@NotNull final RecyclerView.ViewHolder holder, final int position) {
        if (getItemViewType(position) == PictureConfig.TYPE_CAMERA) {
                ...
                if (PictureSelectionConfig.uiStyle != null) {
                    if (isHasVideo) {
                        if (PictureSelectionConfig.uiStyle.picture_adapter_item_video_textLeftDrawable != 0) {
                            ...
                            contentHolder.tvDuration.setCompoundDrawablesRelativeWithIntrinsicBounds
                                    (MResource.getIdentifier(context,"drawable","picture_icon_video"), 0, 0, 0);
                        }
                    } else {
                        if (PictureSelectionConfig.uiStyle.picture_adapter_item_audio_textLeftDrawable != 0) {
                            ...
                            contentHolder.tvDuration.setCompoundDrawablesRelativeWithIntrinsicBounds
                                    (MResource.getIdentifier(context,"drawable","picture_icon_audio"), 0, 0, 0);
                        }
                    }
                } else {
                    contentHolder.tvDuration.setCompoundDrawablesRelativeWithIntrinsicBounds
                            (isHasVideo ? MResource.getIdentifier(context,"drawable","picture_icon_video") : MResource.getIdentifier(context,"drawable","picture_icon_audio"),
                                    0, 0, 0);
                }
            } else {
            ...
            if (config.chooseMode == PictureMimeType.ofAudio()) {
                contentHolder.ivPicture.setImageResource(MResource.getIdentifier(context,"drawable","picture_audio_placeholder"));
            } else {
                ...
                if (eqResult) {
                    if (PictureMimeType.isHasVideo(image.getMimeType())) {
                        if (config.videoMinSecond > 0 && image.getDuration() < config.videoMinSecond) {
                            ...
                            showPromptDialog(context.getString(MResource.getIdentifier(context,"string","picture_choose_min_seconds"), config.videoMinSecond / 1000));
                            return;
                        }
                        if (config.videoMaxSecond > 0 && image.getDuration() > config.videoMaxSecond) {
                            // The length of the video exceeds the specified length
                            showPromptDialog(context.getString(MResource.getIdentifier(context,"string","picture_choose_max_seconds"), config.videoMaxSecond / 1000));
                            return;
                        }
                    }
        ...
        if (config.isWithVideoImage && config.maxVideoSelectNum > 0) {
            if (getSelectedSize() >= config.maxSelectNum) {
                ...
                contentHolder.ivPicture.setColorFilter(ContextCompat.getColor
                        (context, isSelected ? MResource.getIdentifier(context,"color","picture_color_80") : MResource.getIdentifier(context,"color","picture_color_half_white")), PorterDuff.Mode.SRC_ATOP);
                ...
                if (config.chooseMode == PictureMimeType.ofAll()) {
                    if (PictureMimeType.isHasImage(media.getMimeType())) {
                        ...
                        if (!isSelected && !PictureMimeType.isHasImage(item.getMimeType())) {
                            contentHolder.ivPicture.setColorFilter(ContextCompat.getColor
                                    (context, PictureMimeType.isHasVideo(item.getMimeType()) ? MResource.getIdentifier(context,"color","picture_color_half_white") : MResource.getIdentifier(context,"color","picture_color_20")), PorterDuff.Mode.SRC_ATOP);
                        }
                        ...
                        if (!isSelected && !PictureMimeType.isHasVideo(item.getMimeType())) {
                            contentHolder.ivPicture.setColorFilter(ContextCompat.getColor
                                    (context, PictureMimeType.isHasImage(item.getMimeType()) ? MResource.getIdentifier(context,"color","picture_color_half_white") : MResource.getIdentifier(context,"color","picture_color_20")), PorterDuff.Mode.SRC_ATOP);
                        }
                    ...
                    if (config.chooseMode == PictureMimeType.ofVideo() && config.maxVideoSelectNum > 0) {
                        if (!isSelected && getSelectedSize() == config.maxVideoSelectNum) {
                            contentHolder.ivPicture.setColorFilter(ContextCompat.getColor
                                    (context, MResource.getIdentifier(context,"color","picture_color_half_white")), PorterDuff.Mode.SRC_ATOP);
                        }
    ...
    public class CameraViewHolder extends RecyclerView.ViewHolder {
        ...
        public CameraViewHolder(View itemView) {
            super(itemView);
            tvCamera = itemView.findViewById(MResource.getIdentifier(context,"id","tvCamera"));
            if (PictureSelectionConfig.uiStyle != null) {
                ...
                if (PictureSelectionConfig.uiStyle.picture_adapter_item_camera_text !=0) {
                    ...
                    tvCamera.setText(config.chooseMode == PictureMimeType.ofAudio() ? context.getString(MResource.getIdentifier(context,"string","picture_tape"))
                            : context.getString(MResource.getIdentifier(context,"string","picture_take_picture")));
                }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView) {
            ...
            ivPicture = itemView.findViewById(MResource.getIdentifier(itemView.getContext(),"id","ivPicture"));
            tvCheck = itemView.findViewById(MResource.getIdentifier(itemView.getContext(),"id","tvCheck"));
            btnCheck = itemView.findViewById(MResource.getIdentifier(itemView.getContext(),"id","btnCheck"));
            tvDuration = itemView.findViewById(MResource.getIdentifier(itemView.getContext(),"id","tv_duration"));
            tvIsGif = itemView.findViewById(MResource.getIdentifier(itemView.getContext(),"id","tv_isGif"));
            tvLongChart = itemView.findViewById(MResource.getIdentifier(itemView.getContext(),"id","tv_long_chart"));
            if (PictureSelectionConfig.uiStyle != null) {
                ...
                Drawable checkedStyleDrawable = AttrsUtils.getTypeValueDrawable(itemView.getContext(), MResource.getIdentifier(itemView.getContext(),"attr","picture_checked_style"), MResource.getIdentifier(itemView.getContext(),"drawable","picture_checkbox_selector"));
    ...
    private void changeCheckboxState(ViewHolder contentHolder, LocalMedia image) {
        ...
        if (config.isWithVideoImage) {
            ...
            if (PictureMimeType.isHasVideo(image.getMimeType())) {
                if (config.maxVideoSelectNum <= 0) {
                    showPromptDialog(context.getString(MResource.getIdentifier(context,"string","picture_rule")));
                    return;
                }

                if (count >= config.maxSelectNum && !isChecked) {
                    showPromptDialog(context.getString(MResource.getIdentifier(context,"string","picture_message_max_num"), config.maxSelectNum));
                    return;
                }
        ...
        if (isChecked) {
            ...
            contentHolder.tvCheck.startAnimation(AnimationUtils.loadAnimation(context, MResource.getIdentifier(context,"anim","picture_anim_modal_in")));
        }
    ...
    public void selectImage(ViewHolder holder, boolean isChecked) {
        ...
        if (isChecked) {
            holder.ivPicture.setColorFilter(ContextCompat.getColor
                    (context, MResource.getIdentifier(context,"color","picture_color_80")), PorterDuff.Mode.SRC_ATOP);
        } else {
            holder.ivPicture.setColorFilter(ContextCompat.getColor
                    (context, MResource.getIdentifier(context,"color","picture_color_20")), PorterDuff.Mode.SRC_ATOP);
        }
    }
    ...
    private void showPromptDialog(String content) {
        ToastUtils.s(context, content);
//        PictureCustomDialog dialog = new PictureCustomDialog(context, MResource.getIdentifier(context,"layout","picture_prompt_dialog"));
//        TextView btnOk = dialog.findViewById(MResource.getIdentifier(context,"id","btnOk"));
//        TextView tvContent = dialog.findViewById(MResource.getIdentifier(context,"id","tv_content"));
