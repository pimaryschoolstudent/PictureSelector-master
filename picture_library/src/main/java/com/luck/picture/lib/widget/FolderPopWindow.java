...
public class FolderPopWindow extends PopupWindow {
    ...
    public FolderPopWindow(Context context) {
        ...
        this.window = LayoutInflater.from(context).inflate(MResource.getIdentifier(context,"layout","picture_window_folder"), null);
        this.setContentView(window);
        this.setWidth(RelativeLayout.LayoutParams.MATCH_PARENT);
        this.setHeight(RelativeLayout.LayoutParams.WRAP_CONTENT);
        this.setAnimationStyle(MResource.getIdentifier(context,"style","PictureThemeWindowStyle"));
        ...
        if (PictureSelectionConfig.uiStyle != null) {
            ...
            if (config.isWeChatStyle) {
                this.drawableUp = ContextCompat.getDrawable(context, MResource.getIdentifier(context,"drawable","picture_icon_wechat_up"));
                this.drawableDown = ContextCompat.getDrawable(context, MResource.getIdentifier(context,"drawable","picture_icon_wechat_down"));
            } else {
                if (config.upResId != 0) {
                    this.drawableUp = ContextCompat.getDrawable(context, config.upResId);
                } else {
                    // 兼容老的Theme方式
                    this.drawableUp = AttrsUtils.getTypeValueDrawable(context,MResource.getIdentifier(context,"attr","picture_arrow_up_icon"), MResource.getIdentifier(context,"drawable","picture_icon_arrow_up"));
                }
                if (config.downResId != 0) {
                    ...
                    this.drawableDown = AttrsUtils.getTypeValueDrawable(context, MResource.getIdentifier(context,"attr","picture_arrow_down_icon"), MResource.getIdentifier(context,"drawable","picture_icon_arrow_down"));
                }
            }
        }
    ...
    public void initView() {
        rootViewBg = window.findViewById(MResource.getIdentifier(context,"id","rootViewBg"));
        adapter = new PictureAlbumDirectoryAdapter(config);
        mRecyclerView = window.findViewById(MResource.getIdentifier(context,"id","folder_list"));
        mRecyclerView.setLayoutManager(new LinearLayoutManager(context));
        mRecyclerView.setAdapter(adapter);
        View rootView = window.findViewById(MResource.getIdentifier(context,"id","rootView"));
