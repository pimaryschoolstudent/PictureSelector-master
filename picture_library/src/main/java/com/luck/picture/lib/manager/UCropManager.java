...
public class UCropManager {
    ...
    public static void ofCrop(Activity activity, String originalPath, String mimeType) {
        ...
        if (TextUtils.isEmpty(originalPath)) {
            ToastUtils.s(activity.getApplicationContext(), activity.getString(MResource.getIdentifier(activity,"string","picture_not_crop_data")));
            return;
        }
    ...
    public static UCrop.Options basicOptions(Context context) {
        ...
        if (PictureSelectionConfig.uiStyle != null) {
            ...
            if (!isChangeStatusBarFontColor) {
                isChangeStatusBarFontColor = AttrsUtils.getTypeValueBoolean(context, MResource.getIdentifier(context,"attr","picture_statusFontColor"));
            }
            if (config.cropTitleBarBackgroundColor != 0) {
                toolbarColor = config.cropTitleBarBackgroundColor;
            } else {
                toolbarColor = AttrsUtils.getTypeValueColor(context, MResource.getIdentifier(context,"attr","picture_crop_toolbar_bg"));
            }
            if (config.cropStatusBarColorPrimaryDark != 0) {
                statusColor = config.cropStatusBarColorPrimaryDark;
            } else {
                statusColor = AttrsUtils.getTypeValueColor(context, MResource.getIdentifier(context,"attr","picture_crop_status_color"));
            }
            if (config.cropTitleColor != 0) {
                titleColor = config.cropTitleColor;
            } else {
                titleColor = AttrsUtils.getTypeValueColor(context, MResource.getIdentifier(context,"attr","picture_crop_title_color"));
            }
        }
