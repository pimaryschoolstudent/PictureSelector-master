...
public class PictureAlbumDirectoryAdapter extends RecyclerView.Adapter<PictureAlbumDirectoryAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(MResource.getIdentifier(parent.getContext(),"layout","picture_album_folder_item"), parent, false);
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        if (chooseMode == PictureMimeType.ofAudio()) {
            holder.ivFirstImage.setImageResource(MResource.getIdentifier(holder.itemView.getContext(),"drawable","picture_audio_placeholder"));
        } else {
        ...
        String firstTitle = folder.getOfAllType() != -1 ? folder.getOfAllType() == PictureMimeType.ofAudio() ?
                context.getString(MResource.getIdentifier(context,"string","picture_all_audio"))
                : context.getString(MResource.getIdentifier(context,"string","picture_camera_roll")) : name;
        holder.tvFolderName.setText(context.getString(MResource.getIdentifier(context,"string","picture_camera_roll_num"), firstTitle, imageNum));
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView) {
            super(itemView);
            ivFirstImage = itemView.findViewById(MResource.getIdentifier(itemView.getContext(),"id","first_image"));
            tvFolderName = itemView.findViewById(MResource.getIdentifier(itemView.getContext(),"id","tv_folder_name"));
            tvSign = itemView.findViewById(MResource.getIdentifier(itemView.getContext(),"id","tv_sign"));
            if (PictureSelectionConfig.uiStyle != null) {
                ...
                Drawable folderCheckedDotDrawable = AttrsUtils.getTypeValueDrawable(itemView.getContext(), MResource.getIdentifier(itemView.getContext(),"attr","picture_folder_checked_dot"), MResource.getIdentifier(itemView.getContext(),"drawable","picture_orange_oval"));
                tvSign.setBackground(folderCheckedDotDrawable);
                int folderTextColor = AttrsUtils.getTypeValueColor(itemView.getContext(), MResource.getIdentifier(itemView.getContext(),"attr","picture_folder_textColor"));
                if (folderTextColor != 0) {
                    tvFolderName.setTextColor(folderTextColor);
                }
                float folderTextSize = AttrsUtils.getTypeValueSize(itemView.getContext(), MResource.getIdentifier(itemView.getContext(),"attr","picture_folder_textSize"));
                if (folderTextSize > 0) {
